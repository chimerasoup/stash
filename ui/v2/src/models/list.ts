...
export class Criteria {
  type: CriteriaType = CriteriaType.None;
  valueType: CriteriaValueType = CriteriaValueType.Single;
  options: any[] = [];
  parameterName: string = '';
  value: string = '';
  values: string[] = [];

  async configure(type: CriteriaType) {
    this.type = type;
  ...
  configureFromQueryParameters(params: any) {
    ...
    if (params['c'] != null) {
      ...
      jsonParameters.forEach(jsonString => {
        ...
        criteria.configure(encodedCriteria.type);
